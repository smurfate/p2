...
public final class AndroidHttpClient implements HttpClient {
	...
	// Gzip of items shorter than this probably won't be worthwhile
	 ...
	 * then this will get a stream over the uncompressed items.
	 * 
	 * @param entity
	...
	/**
	 * Compress items to send to server. Creates a Http Entity holding the
	 * gzipped items. The items will not be compressed if it is too short.
	 * 
	 * @param data
	 *            The bytes to compress
	 * @return Entity holding the items
	 */
	...
	/**
	 * Retrieves the minimum size for compressing items. Shorter items will not be
	 * compressed.
	 */
		...
		if (request instanceof HttpEntityEnclosingRequest) {
			...
			if (entity != null && entity.isRepeatable()) {
				if (entity.getContentLength() < 1024) {
					...
					builder.append(" --items-ascii \"").append(entityString)
							.append("\"");
				} else {
