...
final class Utils {
	...
	static boolean isAirplaneModeOn(Context context) {
		...
		try {
			...
			// https://github.com/square/picasso/issues/761, some devices might
			// crash here, assume that
	...
	static boolean isWebPFile(InputStream stream) throws IOException {
		...
		if (stream.read(fileHeaderBytes, 0, WEBP_FILE_HEADER_SIZE) == WEBP_FILE_HEADER_SIZE) {
			...
			// is a WebP file
			isWebPFile = WEBP_FILE_HEADER_RIFF.equals(new String(
					fileHeaderBytes, 0, 4, "US-ASCII"))
					&& WEBP_FILE_HEADER_WEBP.equals(new String(fileHeaderBytes,
							8, 4, "US-ASCII"));
		}
		return isWebPFile;
	}
	...
	/**
	 * Prior to Android 5, HandlerThread always keeps a stack local reference to
	 * the last message that was sent to it. This method makes sure that stack
	 * local reference never stays there for too long by sending new messages to
	 * it every second.
	 */
