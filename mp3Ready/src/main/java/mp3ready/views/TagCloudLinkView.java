...
public class TagCloudLinkView extends RelativeLayout {
	...
	private static final int DEFAULT_TAG_LAYOUT_COLOR = Color
			.parseColor("#aa66cc");
	private static final int DEFAULT_TAG_TEXT_COLOR = Color
			.parseColor("#1a1a1a");
	private static final int DEFAULT_DELETABLE_TEXT_COLOR = Color
			.parseColor("#1a1a1a");
	...
	private void initialize(Context ctx, AttributeSet attrs, int defStyle) {
		...
		TypedArray typeArray = ctx.obtainStyledAttributes(attrs,
				R.styleable.TagCloudLinkView, defStyle, defStyle);
		mTagLayoutColor = typeArray.getColor(
				R.styleable.TagCloudLinkView_tagLayoutColor,
				DEFAULT_TAG_LAYOUT_COLOR);
		mTagTextColor = typeArray.getColor(
				R.styleable.TagCloudLinkView_tagTextColor,
		...
		mDeletableTextColor = typeArray.getColor(
				R.styleable.TagCloudLinkView_deletableTextColor,
				DEFAULT_TAG_TEXT_COLOR);
		mDeletableTextSize = typeArray.getDimension(
				R.styleable.TagCloudLinkView_deletableTextSize,
	...
	public void drawTags() {
		...
		for (Tag item : mTags) {
			...
			tagView.setPadding(INNER_VIEW_PADDING, INNER_VIEW_PADDING,
					INNER_VIEW_PADDING, INNER_VIEW_PADDING);
			tagView.setTextColor(mTagTextColor);
			tagView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTagTextSize);
			tagView.setAlpha(tag.getAlpha());
			...
			// deletable text
			TextView deletableView = (TextView) tagLayout
					.findViewById(R.id.delete_txt);
			if (mIsDeletable) {
				...
				tagWidth += deletableView.getPaint().measureText(
						DEFAULT_DELETABLE_STRING)
						+ INNER_VIEW_PADDING * 2; // deletableView Padding (left
													// & right)
			} else {
				deletableView.setVisibility(View.GONE);
			}

			LinearLayout.LayoutParams tagParams = new LinearLayout.LayoutParams(HEIGHT_WC, HEIGHT_WC);
			tagParams.setMargins(5,5, 0, 5);
			...
			ll_parent.addView(tagLayout,tagParams);
			index++;
			listIndex++;
		}
		addView(hScroll);
