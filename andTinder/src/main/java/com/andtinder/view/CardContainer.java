...
public class CardContainer extends AdapterView<ListAdapter> {
    ...
    public interface OnLastCardDismissedListener {
        void onLastCardDismissed();
    }
    ...
    private OnLastCardDismissedListener listener;
    ...
    public void setOnLastCardDismissedListener(OnLastCardDismissedListener l){
    	this.listener = l;
    }
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (getChildCount() != 0) {
            ...
            //cardModel= adapter.getItem(adapter.getCount()-1);
    ...
    public CardModel getCurrCard()
    {
    	return (CardModel)getAdapter().getItem(getAdapter().getCount() - getChildCount());
    }
    ...
    public void dislike(){
        ...
        //Log.i("CardContainer", "child:"+getChildCount()+",items:"+getAdapter().getCount());
        CardModel model = (CardModel)getAdapter().getItem(getAdapter().getCount() - getChildCount());
        if (model.getOnCardDismissedListener()!=null&&mTopCard!=null){
        	model.getOnCardDismissedListener().onDislike(mTopCard,model);
        }
        ...
        topCard.animate()
            ...
            .x(-Utils.getScreenSizePixels(getContext())[0])
            .y(topCard.getY())
            .rotation(-45)
            .setListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    ensureFull();
                    if (mTopCard==null){
                    	if (listener!=null){listener.onLastCardDismissed();}
                    }
                    animate = false;
                }
    ...
    public void triggerCustomeEvent(int eventType)
    {
        if (mTopCard ==null){return;}
        if (getChildCount()<=0){return;}
        CardModel model = (CardModel)getAdapter().getItem(getAdapter().getCount() - getChildCount());
        if (model.getOnCardDismissedListener()!=null&&mTopCard!=null){
        	model.getOnCardDismissedListener().onCustomEvent(eventType,mTopCard);
        }
    }
    public void like(){
        ...
        CardModel model = (CardModel)getAdapter().getItem(getAdapter().getCount() - getChildCount());
        if (model.getOnCardDismissedListener()!=null&&mTopCard!=null){
        	model.getOnCardDismissedListener().onLike(mTopCard,model);
        }
        //View likeImage = mTopCard.findViewById(R.id.like_text_image);
        //View dislikeImage = mTopCard.findViewById(R.id.dislike_text_image);
        //dislikeImage.setVisibility(View.VISIBLE);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (!boundsRect.contains((int) x, (int) y)) {
                    return false;
                }
                mLastTouchX = x;
                mLastTouchY = y;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mTopCard.setRotation(10 * mTopCard.getTranslationX() / (getWidth() / 2.f));

                mLastTouchX = x;
                mLastTouchY = y;
                CardModel cardModel = (CardModel)getAdapter().getItem(getAdapter().getCount() - getChildCount());
                if (Math.abs(getWidth()-x) >= (getWidth()-(getWidth()/4))){
                	if (cardModel.getOnCardDismissedListener()!=null)
                		cardModel.setTendToLike(true);
                		cardModel.getOnCardDismissedListener().onTendToDislike(mTopCard);
                }else if (Math.abs(getWidth()-x) <= (getWidth()/4)){
                	if (cardModel.getOnCardDismissedListener()!=null)
                		cardModel.setTendToLike(false);
                		cardModel.getOnCardDismissedListener().onTendToLike(mTopCard);
                }else {
                	cardModel.getOnCardDismissedListener().onNoTend(mTopCard);
                }
                break;
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                animator.start();
                CardModel mModel = (CardModel)getAdapter().getItem(getAdapter().getCount() - getChildCount());
               if (mModel.getOnCardDismissedListener()!=null){
            	   mModel.getOnCardDismissedListener().onNoTend(mTopCard);
               }
                break;
    ...
    private class GestureListener extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (Math.abs(dx) > mTouchSlop &&
                ...
                if (cardModel.getOnCardDismissedListener() != null) {
                    if ( targetX > 0 ) {
                    	//Log.i("CardContainer", cardModel.getCardId()+":"+cardModel.getTitle());
                        cardModel.getOnCardDismissedListener().onLike(prev,cardModel);
                    } else {
                        cardModel.getOnCardDismissedListener().onDislike(prev,cardModel);
                    }
                }
                animate = true;
                topCard.animate()
